...
class ProductForm(Toplevel):
    def __init__(self, parent):
        ...
        self.frame1 = Frame(self, padding=15, border=10,
        ...
        self.LinkImg = Image.open("./img/icon-search.png")
        ...
        self.lb1 = Label(self.frameLabel1, text="Hãng sản xuất", padding=12, font=(
            "Times New Roman", 16), background="gray")
        self.lb1.pack()
        # Tạo frame label2
        self.frameLabel2 = Frame(self.frameLabel, borderwidth=1, relief=SOLID)
        self.frameLabel2.pack(side=LEFT, padx=15, pady=15)
        self.lb2 = Label(self.frameLabel2, text="Tất cả sản phẩm", padding=12, font=(
            "Times New Roman", 16), background="gray", anchor="c", width=57)
        self.lb2.pack()
        # Tao frame Label3
        self.frameLabel3 = Frame(self.frameLabel)
        self.frameLabel3.pack(side=LEFT, padx=15, pady=15)
        self.lb3 = Label(self.frameLabel3, text="Giỏ hàng", padding=12, font=(
            "Times New Roman", 16), background="gray", anchor="c", width=100)
        self.lb3.pack()
        # Kết thúc tạo frame label
        ...
        # self.frame_canvas.config(width=680 + self.vsb.winfo_width(),
        #                     height= 500)
        # chuẩn ban đầu
        self.frame_canvas.config(width=652 + self.vsb.winfo_width(),
                                 height=400)
        # Set the canvas scrolling region
        self.canvas.config(scrollregion=self.canvas.bbox("all"))
        # end frame list product
        ...
        # self.frameProduct = Frame(self.frameListCart)
        # self.frameProduct.pack(side=LEFT)
        ...
        # # Link a scrollbar to the canvas
        self.vsbCart = Scrollbar(
            self.frame_canvasCart, orient="vertical", command=self.canvasCart.yview)
        ...
        self.canvasCart.create_window(
            (0, 0), window=self.frame_buttonsCart, anchor='nw')

        # xử lý cart
        self.listCart = []

        # self.frame_buttonsCart.update_idletasks()
        self.frame_canvasCart.config(width=330 + self.vsbCart.winfo_width(),
                                     height=400)

        # Tạo frame show tổng tiền và button đặt hàng
        self.frameTotalGrid = Frame(self.frameListCart)
        self.frameTotalGrid.grid(pady=5)
        self.lbTotal = Label(self.frameTotalGrid,
                             text="Tổng tiền:", font=('TimesNewRoman 12 bold'))
        self.lbTotal.grid(row=0, column=0)
        self.lbTotalPrice = Label(
            self.frameTotalGrid, text="", font=("Times New Roman", 14))
    ...
    # Phần dưới là các hàm xử lý logic
        ...
        for i in range(0, rows):
            for j in range(0, columns):
                ...
                if (count < len(filtered_list)):
                    ...
                    # end xử lý image
                    productName = Label(
                        item, text=f'{filtered_list[count][1]}', font=("Times New Roman", 12))
                    productName.grid(row=1, column=0)

                    productPrice = Label(
                        item, text=f'Giá: {filtered_list[count][2]}', font=("Times New Roman", 12))
                    productPrice.grid(row=2, column=0)

                    n = StringVar()
                    productCombobox = Combobox(
                        item, textvariable=n, state='readonly')

                    # Adding combobox drop down list
                    arrSizeOfProduct = objProduct.getSizeAndQuantity(
                    ...
                    # btn= Button(item,text="Add cart",command=lambda productImg=listProduct[count][0],name=listProduct[count][1],price=listProduct[count][2], size=productCombobox.get() : self.handleEventCart(productImg,name,price,"39"))

                    # By using lambda function, the value of selected_value is evaluated at the time the Add cart button is clicked, so it will always get the current value of the productCombobox.

                    btn = Button(item, text="Add cart", command=self.getValueClicked(
                        f"./img/product/{filtered_list[count][0]}.png", filtered_list[count][1], filtered_list[count][2], n))
        ...
        for i in range(0, rows):
            for j in range(0, columns):
                ...
                if (count < len(listProduct)):
                    ...
                    # end xử lý image
                    productName = Label(
                        item, text=f'{listProduct[count][1]}', font=("Times New Roman", 12))
                    productName.grid(row=1, column=0)

                    productPrice = Label(
                        item, text=f'Giá: {listProduct[count][2]}', font=("Times New Roman", 12))
                    productPrice.grid(row=2, column=0)
                    ...
                    btn = Button(item, text="Add cart", command=self.getValueClicked(
                        f"./img/product/{listProduct[count][0]}.png", listProduct[count][1], listProduct[count][2], n))
    ...
    def renderListCart(self):
        ...
        for index, value in enumerate(self.listCart):
            ...
            btnDelete = Button(itemCart, text="Xóa", font=(
                "Times New Roman", 10), command=self.getValueDel(value[1], value[3]))
            btnDelete.grid(column=0, row=3, pady=(8, 0))

            productName = Label(
                itemDetail, text=value[1], font=("Times New Roman", 12))
            productName.grid(row=0, column=0)
            productPrice = Label(
                itemDetail, text=f'Giá: {value[2]}', font=("Times New Roman", 12))
            productPrice.grid(row=1, column=0)
            productSize = Label(
                itemDetail, text=f'Size: {value[3]}', font=("Times New Roman", 12))
            productSize.grid(row=2, column=0)
            productAmount = Label(
                itemDetail, text=f'Số lượng: X{value[4]}', font=("Times New Roman", 12))
    ...
    # Hàm xử lý logic xóa sản phẩm trong giỏ hàng
...
if __name__ == '__main__':
    screen = ProductForm()
    screen.geometry("1200x600+180+100")
